...
class PlayerDal(BaseDAL):
    ...
    def get_player_by_id(self, player_id: int) -> Optional[Players]:
        return self.session.query(Players).filter_by(player_id=player_id).first()

    def get_player_by_api_id_and_sport_id(self, player_api_id: int, player_sport_id: int):
        return self.session.query(Players).filter_by(api_id=player_api_id, sport_id=player_sport_id).first()

    def get_player_by_name_and_sport_id(self, player_name: str, player_sport_id: int):
        return self.session.query(Players).filter_by(name=player_name, sport_id=player_sport_id).first()
