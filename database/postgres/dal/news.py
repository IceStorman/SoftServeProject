...
class NewsDAL:
    ...
    def get_user_interactions_with_news_by_period_of_time(self, user_id: int, period_of_time: int = PERIOD_OF_TIME) -> list[tuple]:
        ...
        likes_query = (
            ...
            .filter(
                    InteractionWithNews.timestamp >= period_of_time,
                    InteractionWithNews.user_id == user_id,
                    InteractionWithNews.interaction_type == 1
            )
        )
        ...
        views_query = (
            ...
            .filter(
                    InteractionWithNews.timestamp >= period_of_time,
                    InteractionWithNews.user_id == user_id,
                    InteractionWithNews.interaction_type == 2
            )
        )
