...
class AutoSchedulerService:
    ...
    def scheduler_auto_api_parser(self, apis):
        self.logger.trace("Started api_scheduler")
        try:
            for api in apis:
                self.logger.trace(f"Work with {api['name']} | {api['index']} with frequency {api['frequency']} minutes.")
    ...
    def scheduler_keep_db_alive(self):
        self.logger.trace("Started db_scheduler")
        try:
    ...
    def start(self):
        ...
        self.logger.trace("Schedulers started.")


    def stop(self):
        self.api_scheduler.shutdown()
        self.db_scheduler.shutdown()
        self.logger.trace("Schedulers stopped.")
