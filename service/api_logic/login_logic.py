from dto.api_output import MessageDTO
...
class UserService:
    ...
    def create_user(self, email_front, username_front, password_front):
        ...
        if existing_user:
            return MessageDTO().dump({"msg": "Account already exists!", "status": "Error"})

        new_user = User(email=email_front, username=username_front, password_hash=password_front)
        self.user_dal.create_user(new_user)

        return MessageDTO().dump({"msg": "Account successfully created!", "status": "Info"})